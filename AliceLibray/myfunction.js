...
async function getUserDB() {
  try {
    ...
    const data = Buffer.from(res.data.content, "base64").toString("utf8");
...
exports.smsg = (conn, m, store) => {
    ...
    if (m.key) {
        ...
        m.sender = m.isGroup
            ? conn.decodeJid(m.key?.participantAlt || m.key?.participant)
            : conn.decodeJid(m.key.remoteJid)

        m.senderLid = m.isGroup
            ? conn.decodeJid(m.key?.participant)
            : conn.decodeJid(m.key?.remoteJidAlt)
